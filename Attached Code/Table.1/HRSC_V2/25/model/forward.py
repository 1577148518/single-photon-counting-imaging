...
def img_recom(x,is_combine=True):
	div_num = int(FLAGS.div_fac*4)
	print("div_num:",div_num)
	img_reg = tf.split(x, num_or_size_splits=div_num, axis=1)
	for j in range(div_num):
		img_reg[j] = tf.split(img_reg[j], num_or_size_splits=div_num, axis=2)
	...
	for i in range(div_num):
		...
		for j in range(div_num):
			...
			j_div = j//4
			if is_combine:
				#print("left:",i_rmd*FLAGS.div_fac+i_div,j_rmd*FLAGS.div_fac+j_div,"right:",i,j)
				mid = img_reg_save[i][j]
				img_reg[i_rmd*FLAGS.div_fac+i_div][j_rmd*FLAGS.div_fac+j_div] = mid
			else:
				#print("left:",i,j,"right:",i_rmd*FLAGS.div_fac+i_div,j_rmd*FLAGS.div_fac+j_div)
				mid = img_reg_save[i_rmd*FLAGS.div_fac+i_div][j_rmd*FLAGS.div_fac+j_div]
				img_reg[i][j] = mid 
	
	i_matrix = [0 for i in range(div_num)]
	for i  in range(div_num):
		j_matrix = img_reg[i]
		i_matrix[i] = j_matrix[0]
		for j in range(1,div_num):
...
def forward(x,regularizer):
	...
	pixel1 = shift_ps(conv1,16,16)             # h*w等于采样次数
	print("pixel1",pixel1)
	
	recov_conv1_w = get_weight([16,16,1,1024],FLAGS.REGULARIZER)            #
	print("recov_conv1_w:",recov_conv1_w)
	recov_conv1_b = get_bias([1024])
	recov_conv1 = tf.nn.relu(tf.nn.conv2d(pixel1,recov_conv1_w,strides=[1,16,16,1], padding='VALID')+recov_conv1_b)    #
